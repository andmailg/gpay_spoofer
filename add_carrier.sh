...
# Формируем строку по нашему стандарту
NEW_LINE="${NEW_ID}:${INPUT_NUMERIC}:${INPUT_ISO}:${INPUT_NAME}"

# --- Безопасная запись в базу данных ---
# Перенаправляем вывод через временный файл, чтобы toybox sed не повредил chown/chmod оригинального файла
